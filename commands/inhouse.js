...
module.exports.run = async(bot, message, args) => {
	...
	if(args.length <= 0) {
		return message.channel.send({embed: {
			...
			title: 'Yikes',
			description: "Type ~inhouse help to learn how to use this command!",
			timestamp: new Date(),
			footer: {
				icon_url: bot.user.avatarURL,
				text: "© WilsonBot Co."
	...
	if(args[0] == 'add') {
		...
		function finished(err) { console.log(err); }
		...
		if(args[1] == 'roles' || args[1] == 'role') {
			if(args.length != 8) {
				return message.channel.send({embed: {
					...
					fields: [{
							name: "Command Usage",
							value: "~inhouse update roles WilsonBot [role] [role] [role] [role] [role]"
							},
							{
							name: "Rank Examples",
			...
			if(!players[args[2]]) {
				return message.channel.send({embed: {
					...
					title: 'Yikes',
					description: "Player " + args[2] + "doesn't exist! Make sure you're using the correct casing.",
					timestamp: new Date(),
					footer: {
						icon_url: bot.user.avatarURL,
						text: "© WilsonBot Co."
			...
			return message.channel.send({embed: {
			    ...
			    description: "Here is information on the player you updated.",
			    fields: [{
			        name: "Name",
	...
	if(args[0] == 'teams') {
		...
		let teamOne = [];
		let teamTwo = [];
		let playersToBeUsed = [players[args[1]], players[args[2]], players[args[3]], players[args[4]], players[args[5]], players[args[6]], players[args[7]], players[args[8]], players[args[9]], players[args[10]]];
		for(var i = 0; i < playersToBeUsed.length; i++) {
			if(!playersToBeUsed[i]) {
				return message.channel.send({embed: {
					...
					title: 'Yikes',
					description: "Player " + args[i + 1] + " doesn't exist!",
					timestamp: new Date(),
					footer: {
						icon_url: bot.user.avatarURL,
						text: "© WilsonBot Co."
		...
		while(teamOne.length > 0) {
			...
			if(teamOneRoles[placePlayer.roles.first] == 'free') teamOneRoles[placePlayer.roles.first] = placePlayer;
			else if(teamOneRoles[placePlayer.roles.second] == 'free') teamOneRoles[placePlayer.roles.second] = placePlayer;
			else if(teamOneRoles[placePlayer.roles.third] == 'free') teamOneRoles[placePlayer.roles.third] = placePlayer;
			else if(teamOneRoles[placePlayer.roles.fourth] == 'free') teamOneRoles[placePlayer.roles.fourth] = placePlayer;
			else if(teamOneRoles[placePlayer.roles.fifth] == 'free') teamOneRoles[placePlayer.roles.fifth] = placePlayer;
			else { console.log('Uh oh.' + placePlayer.name); }
		}
		while(teamTwo.length > 0) {
			var placePlayer = teamTwo.pop();
			if(teamTwoRoles[placePlayer.roles.first] == 'free') teamTwoRoles[placePlayer.roles.first] = placePlayer;
			else if(teamTwoRoles[placePlayer.roles.second] == 'free') teamTwoRoles[placePlayer.roles.second] = placePlayer;
			else if(teamTwoRoles[placePlayer.roles.third] == 'free') teamTwoRoles[placePlayer.roles.third] = placePlayer;
			else if(teamTwoRoles[placePlayer.roles.fourth] == 'free') teamTwoRoles[placePlayer.roles.fourth] = placePlayer;
			else if(teamTwoRoles[placePlayer.roles.fifth] == 'free') teamTwoRoles[placePlayer.roles.fifth] = placePlayer;
			else { console.log('Uh oh.'+ placePlayer.name); }
		}
		...
		for(var i = 0; i < teamOne.length; i++) { teamOne[i] = teamOne[i].name + ' - ' + teamOne[i].rank.toString(); }
		for(var i = 0; i < teamTwo.length; i++) { teamTwo[i] = teamTwo[i].name + ' - ' + teamTwo[i].rank.toString(); }
		var teamOneString = 'Top: ' + teamOne[0] + '\nJungle: ' + teamOne[1] + '\nMid: ' + teamOne[2] + '\nBottom: ' + teamOne[3] + '\nSupport: ' + teamOne[4];
		var teamTwoString = 'Top: ' + teamTwo[0] + '\nJungle: ' + teamTwo[1] + '\nMid: ' + teamTwo[2] + '\nBottom: ' + teamTwo[3] + '\nSupport: ' + teamTwo[4];
		return message.channel.send({embed: {
			color: 3447003,
			author: {
				name: bot.user.username,
	...
	if(args[0] == 'info') {
		...
		if(!players[args[1]]) {
			return message.channel.send({embed: {
					...
					title: 'Yikes',
					description: "Player " + args[1] + " doesn't exist! Make sure you're using the correct casing.",
					timestamp: new Date(),
					footer: {
						icon_url: bot.user.avatarURL,
						text: "© WilsonBot Co."
...
function translateRoleToNumber(role) {
	if(role.includes('top')) return 0;
	else if(role.includes('j')) return 1;
	else if(role.includes('mid')) return 2;
	else if(role.includes('bot') || role.includes('adc')) return 3;
...
function rankToNumber(rank) {
	...
	if(rank.length == 2) {
		...
		if(rank.charAt(1) == '5' || rank.substring(1).toLowerCase() == 'v') rankNumber += 1;
		else if(rank.charAt(1) == '4' || rank.substring(1).toLowerCase() == 'iv') rankNumber += 2;
		else if(rank.charAt(1) == '3' || rank.substring(1).toLowerCase() == 'iii') rankNumber += 3;
		else if(rank.charAt(1) == '2' || rank.substring(1).toLowerCase() == 'ii') rankNumber += 4;
		else if(rank.charAt(1) == '1' || rank.substring(1).toLowerCase() == 'i') rankNumber += 5;
		else { console.log('Uh oh.'); }
	}
	else {
